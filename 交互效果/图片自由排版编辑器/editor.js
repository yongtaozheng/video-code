...
function applyCanvasColor() {
  stage.style.backgroundColor = isCanvasTransparent
    ? "transparent"
    : canvasColor;
}
...
function applyBackground() {
  stage.style.backgroundImage = backgroundSrc
    ? `url("${backgroundSrc}")`
    : "none";
}
...
function onMouseMove(event) {
  ...
  if (interactionState.type === "drag") {
    ...
    if (direction.includes("w")) {
      left = clamp(start.x + dx, 0, start.right - MIN_LAYER_SIZE);
    }
    ...
    if (direction.includes("n")) {
      top = clamp(start.y + dy, 0, start.bottom - MIN_LAYER_SIZE);
    }
...
bgUpload.addEventListener("change", async (event) => {
  ...
  applyBackground();
  setStatus(
    `背景已更新：${file.name}（画布 ${nextSize.width} x ${nextSize.height}）`,
  );
  bgUpload.value = "";
});
